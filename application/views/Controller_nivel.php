	...
	public function process($PersonaID,$CicloEscolarID){		
		...
		switch ($this->input->post('ctAct')) {
			...
			case 'agregar':			
				...
				$existe = $this->Model_nivel->comparaRelacion($ctID,$PersonaID,$CicloEscolarID);
				if (!$existe) {
					$ok = $this->Model_nivel->agregarrelacion($ctID,$PersonaID,$CicloEscolarID);
					return $ok;
				} else {
					return FALSE;
				}
			break;
			case 'activar':
			$ok=$this->Model_nivel->negar_cts($PersonaID,$CicloEscolarID);
